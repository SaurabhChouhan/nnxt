...
class EstimationTask extends React.PureComponent {
    render() {
            ...
            if (task.owner == SC.OWNER_ESTIMATOR) {
                if (task.addedInThisIteration) {
                    ...
                    if (task.estimator.removalRequested) {
                        ...
                        buttons.push(<img key="he_requested_edit" src="/images/requested_delete.png"></img>)
                    } else {
        ...
        if (task.feature && task.feature._id) {
            ...
            buttons.push(<img key="move_to_feature" src="/images/move_to_feature.png" onClick={()=>{this.props.showFeatureSelectionForm(this.props.task);
