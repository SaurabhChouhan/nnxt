...
leaveSchema.statics.leaveRaised = async (leaveInput, user, schemaRequested) => {
    ...
    let startDateMoment = momentTZ.tz(startDateString, SC.DATE_FORMAT, SC.UTC_TIMEZONE).hour(0).minute(0).second(0).millisecond(0)
    let endDateMoment = momentTZ.tz(endDateString, SC.DATE_FORMAT, SC.UTC_TIMEZONE).hour(0).minute(0).second(0).millisecond(0)
